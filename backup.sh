...
# Run the mysqldump command inside the MySQL container and upload directly to S3
docker exec -i ${MYSQL_CONTAINER} mysqldump -u${DB_USER} -p"${DB_PASSWORD}" ${DB_NAME} | aws s3 cp - "s3://${S3_BUCKET}/${BACKUP_FILENAME}"

# Check if the backup was successful
if [ $? -eq 0 ]; then
  echo "Database backup completed successfully. Backup file uploaded to S3: s3://${S3_BUCKET}/${BACKUP_FILENAME}"
